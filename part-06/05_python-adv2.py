""" Project: Flashcards Learning App """
...
def save_flashcards(flashcards, file_path = "part-06/flashcards.json"):
...
def mark_as_learned():
    ...
    for card in flashcards:
        if not card['learned']:
            ...
            mark = input("Mark this card as leanred? (y/n): ").lower()
            if mark == 'y':
                card['learned'] = True
                save_flashcards(flashcards)
...
def main():
    print("\nWelcome to the Flashcards Learning App!")
    while True:
        print("\nOptions:")
